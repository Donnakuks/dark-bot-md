   ...
   if (pairingCode && !GlobalTechInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +2349137178897")))
            process.exit(0)
         }
      } else {
...
GlobalTechInc.ev.on("connection.update",async  (s) => {
    ...
    whitespaceBreak: false
	        ...
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: davvy0989`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: Donnakuks`))
